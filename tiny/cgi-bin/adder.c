...
int main(void){
    ...
    sprintf(content, "<p>QUERY_STRING=%s", buf);  // ""안의 string이 content라는 string에 쓰인다.
    sprintf(content, "%s</p>Welcome to add.com: ", content);
    sprintf(content, "%sTHE Internet addition portal.\r\n</p>", content);
    sprintf(content, "%sThe answer is: %d + %d = %d\r\n</p>", content, n1, n2, n1+n2);  // 인자를 받아 처리를 해줬다! -> 동적 컨텐츠
    sprintf(content, "%sThanks for visiting! ._.\r\n</p>", content);
